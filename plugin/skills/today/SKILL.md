...
Summary line counts:
- `N active threads (M P0).` — N = count of meta.md with `status` in `{new, to_apply, applied, interviewing}`. M = subset of those with tier P0.
- `K decided this search.` — K = count of meta.md with `status` in `{offer, rejected, closed}`.
- `Warm-outreach attempts this week: A / B.` — A = distinct warm-outreach mentions in the last 7 days of journal.md (keywords: `DM`, `outreach`, `coffee`, `intro`, `intro ask`, `reached out`, `messaged`, `connect`; each dated entry contributes at most one). B = `warm_outreach` target from strategy.md. Omit the entire line if no warm_outreach target set.
