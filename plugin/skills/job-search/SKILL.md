...
### C. Resolve `## Companies of interest`

**Seed Discovery with `## Companies of interest` from `userdata/profile.md`.**

Three cases, checked here in the main conversation — before the subagents are
dispatched:
...
3. Score each new candidate using the tier rubric in `userdata/profile.md` (same logic as `/evaluate-position`). Fetching the JD is part of scoring — a score invented without reading the posting is worthless. Handle a failed fetch by what the failure proves, not by defaulting to a stub:

   - **Posting confirmed gone** — the board's own public API says so (Ashby's org job list omits the ID; Greenhouse/Lever return 404 or an empty posting list), or the page renders an explicit "no longer available" / "position has been filled". **Drop it. Do not file, do not stub.** Record it in the seen-ledger so it never resurfaces. A dead posting is not a lead.
   - **Fetch failed but the posting may exist** — a 403, a bot wall, a JS-only shell you cannot read, a timeout. File a stub with `tier: unscored` and a note saying why it could not be read, so the user can open it themselves.
   - **Never score from the search-result snippet alone.** If you could not read the JD, the candidate is unscored — say so rather than guessing a tier from the title.

   Expect this to bite: `site:`-scoped search returns stale index entries, and a large share of discovered URLs are dead by the time they are fetched. In a 2026-08-04 live run, 26 of 51 discovered postings could not be fetched, most of them verified gone via the board's own API. That verification is what keeps dead links out of the pipeline — it is load-bearing, not a nicety, and it is why the Playwright pass below is only ever an extra.
...
Companies of interest are resolved in Phase 0 (step C). Discovery receives the resolved company list as additional targets — treat them as candidates like any other: they still go through scoring and dedup. After the first run the entries remain in `profile.md` but Discovery need not re-treat them as seeds — they'll be tracked under `userdata/companies/` going forward.
