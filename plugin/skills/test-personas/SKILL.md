...
- **Slash-command discoverability in sub-agents is unverified.** The plugin-under-test agent is told it can invoke `/pm-job-search:<skill>` directly if loaded; whether sub-agents inherit the parent's installed plugins is environment-dependent. The fallback (executing the inlined SKILL.md verbatim) is the safe path. Before relying on this harness as a release gate, verify the discoverability behavior in your target Claude Code version. Tackling this unlocks every other gap because the verification path runs through it.
- **Anti-leak rule is unverified end-to-end.** The 2026-05-27 smoke test surfaced a `Q<N>:` prefix leak in plugin sub-agent output. The fix (anti-leak rule in the plugin-prompt template, commit d0b09f1) landed but has not been exercised on a fresh run — the 2026-06-04 verification re-used the pre-fix transcript and rediscovered the old leak. Verify the rule works once discoverability is resolved by running a fresh cold-start journey and checking the new transcript for `Q\d+:` prefixes.
- **Full 30-turn cold-start completion.** Smoke test stopped at turn 10 (mid-/setup). Validates sustained execution and skill transitions (/setup → /job-search → /dashboard → /today). Depends on discoverability resolution.
- **Three other journeys untested end-to-end.** Active-loop, reflection, edge-recovery have only been validated as journey files + spec criteria; the conversation loop + judge against them has not been exercised. Depends on cold-start completion.
- **Dashboard skill in sub-agent context.** `/pm-job-search:dashboard` launches a local Python server and browser. In sub-agent context it will likely error or hang. Cold-start and edge-recovery journeys invoke it; expect graceful-error behaviour to show up as a soft issue in judge findings rather than a transcript crash. Verified incidentally during cold-start full-run.
