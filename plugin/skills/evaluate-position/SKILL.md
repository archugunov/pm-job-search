...
**Never** overwrite a `not_interested` or `rejected` row's status — those are real history. Re-evaluation of a decided role can update score/research but leaves status as-is.
...
- **What they sell:** <one-line product + business model>
- **The role:** <one-line position scope — what surface, what trade-offs>

## Why this fits

**<Tier verdict line — single bolded sentence that captures the score, e.g.
"Strong P0." / "P1 with upside." / "P2 — surface fit is exact, org shape is
the problem.">**

- **<Category>:** <one-line bullet anchored to a tier_weights dimension or
  positioning angle. Typical categories: Domain / Skills carry over /
  Role-shape / Size + stage / Geography / Positioning fit.>
- **<Category>:** <same. Aim for 3-5 bullets.>
- **<Honest gap / risk / verdict>:** <last bullet acknowledges what's
  adjacent vs. exact, or sets expectation for next step. Don't oversell.>

## Open questions

- <Lead with the noun, not "How is...". e.g. "Team shape — IC role with eng
  partners, or people-management with squad leads underneath?">
- <2-4 bullets total.>
```

Hard rules:
- Maximum ~250 words total. If the brief grows, cut "Open questions" first, then trim "Why this fits."
...
- Never overwrites `not_interested` or `rejected` status during re-evaluation.
