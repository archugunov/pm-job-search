...
import { companyDocMdComponents } from "./briefMarkdown";
...
type TabKey = "research" | "notes" | "prep" | "debriefs";

export function CompanyDrawer({ opened, onClose, folderPath, company, position, hasNotes, link }: Props) {
  const [activeTab, setActiveTab] = useState<TabKey>(hasNotes ? "notes" : "research");
  ...
  // Pick the default tab whenever a different position is opened: Notes if it
  // already has any, otherwise Research. Anchored on folderPath so the user's
  // mid-session tab choice isn't overridden if hasNotes later changes.
  useEffect(() => {
    if (!folderPath) return;
    setActiveTab(hasNotes ? "notes" : "research");
    ...
    <Drawer
      ...
      title={
        <Stack gap={0}>
          ...
          {position && <Text size="xs" c="dimmed">{position}</Text>}
        </Stack>
      }
      position="right"
      size="md"
      ...
      <Stack gap="md">
        {link && (
          <Group gap={6} align="center">
            <IconExternalLink size={14} stroke={1.6} color="var(--mantine-color-dimmed)" />
            <Text
              component="a"
              href={link}
              target="_blank"
...
function Markdown({ children }: { children: string }) {
  return (
    <ReactMarkdown remarkPlugins={[remarkGfm]} components={companyDocMdComponents}>
      {children}
    </ReactMarkdown>
  );
}
...
// Shared renderer for prep and debrief lists. Behaviour scales with count:
//   - 1 doc:  rendered inline without accordion chrome — no point in a
//             collapsible widget for a single item.
//   - 2+ docs: accordion with the newest doc expanded by default. Multiple
//             panels can be open at once so the user can compare e.g. two
//             debriefs from the same round.
function DatedDocs({ docs }: { docs: DatedDoc[] }) {
  if (docs.length === 1) {
    const doc = docs[0];
    return (
      <Stack gap={4}>
        <DocHeader date={doc.date} badge={doc.badge} dim />
  ...
  return (
    <Accordion
      multiple
      defaultValue={[docs[0].key]}
      variant="default"
      chevronPosition="right"
    >
      {docs.map((doc) => (
        <Accordion.Item key={doc.key} value={doc.key}>
          <Accordion.Control px={0}>
            <DocHeader date={doc.date} badge={doc.badge} />
          </Accordion.Control>
          <Accordion.Panel>
            <Markdown>{doc.markdown}</Markdown>
          </Accordion.Panel>
...
function DocHeader({
  ...
  return (
    <Group gap="xs" align="center" wrap="nowrap">
      <Text size={dim ? "xs" : "sm"} fw={500} c={dim ? "dimmed" : undefined}>
        {date}
      </Text>
      {badge && (
