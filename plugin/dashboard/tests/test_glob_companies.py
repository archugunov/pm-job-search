...
def test_passes_through_status_specific_fields(tmp_path: Path):
    """Rejection / not_interested / next_event fields must pass through to API records.

    Previously these were dropped by _OPTIONAL_FIELDS whitelist, leaving the
    dashboard unable to render rejection-stage / not-interested-date / next-event signals.
    """
    companies = tmp_path / "companies"
    ...
    not_interested_path = companies / "Replit" / "meta.md"
    not_interested_path.parent.mkdir(parents=True, exist_ok=True)
    not_interested_path.write_text(
    ...
    assert replit["date_not_interested"] == "2026-05-03"
