...
Synthetic reference install for pm-job-search. Demonstrates a lived-in state at 2026-08-03: London-based Senior PM / HoP candidate in UK fintech with 1 offer in hand (Lendable), 4 active interviews (Plaid CC, Brex, Mercury, Cleo), 4 applied (Marshmallow, Ramp, Anthropic Lead PM Claude Code, iwoca), 2 to-apply (Plaid Growth Loops, Zopa), and 5 decided (2 rejected, 3 not-interested). Weekly reflection happened last week (2026-07-27).
...
- Stale applied bullet — fires for iwoca (`applied`, `date_applied` 2026-07-08 = 26d ago, >14d threshold).
...
- Calendar-event reference — Lendable's `next_event: "Reference call Wed 2026-08-05 11:00"` is 2 days out from today; would surface in Heads-up if calendar were wired (it isn't in this install — see "What's NOT wired" below).
...
None of Trigger A, B, or C fires from this install at 2026-08-03 as shipped. Each is reachable with a small fixture tweak — useful when manually testing the coach-handoff code path.

- **Trigger A** (cadence drift, >50% application miss for 3 weeks running). Actual windows ending 2026-08-03: 2/3 (not a miss), 1/3 (miss), 1/3 (miss) — two consecutive misses, not three, so does NOT fire. To exercise: delete or shift the `date_applied` of either Marshmallow (2026-07-29) or Ramp (2026-07-28) so the most-recent window also misses.
...
- `strategy.md` — target_offer_date 2026-10-17, weekly_targets (5 outreach, 3 applications), 5 populated anti-goals.
